...
pipeline {
  ...
  environment {
    ...
    JIRA_NUXEO_ADDON_MOVING_VERSION = 'compound-2023.x'
      ...
      steps {
        container('maven') {
          script {
            ...
            nxMvn.download(artifact: "org.nuxeo.compound.documents:nuxeo-compound-documents-package:${BUILD_VERSION}:zip")
            nxUtils.postForm(credentialsId: 'connect-prod', url: "${CONNECT_PROD_SITE_URL}marketplace/upload?batch=true",
                form: ["package=@target/nuxeo-compound-documents-package-${BUILD_VERSION}.zip"])
          }
        }
      }
    }
  ...
  post {
    success {
      script {
        ...
        nxSlack.success(message: "Successfully released nuxeo/nuxeo-compound-documents ${VERSION}: ${BUILD_URL}")
      }
    }
    unsuccessful {
      script {
        nxSlack.error(message: "Failed to release nuxeo/nuxeo-compound-documents ${VERSION}: ${BUILD_URL}")
      }
    }
  }
}
