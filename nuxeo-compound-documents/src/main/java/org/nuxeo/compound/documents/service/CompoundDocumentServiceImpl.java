...
public class CompoundDocumentServiceImpl extends DefaultComponent implements CompoundDocumentService {
    ...
    protected void createEntry(CoreSession session, DocumentModel compoundDoc, ZipFile zip, ZipEntry entry,
        ...
        if (entry.isDirectory()) {
            ...
            // addCompoundDocumentFileDefinition(compoundDoc, session.getLastDocumentVersion(doc.getRef()).getId(),
            // doc.getVersionLabel(), doc.getPathAsString(),
            // doc.getPath().removeFirstSegments(compoundDoc.getPath().segmentCount() - 1).toString());
    ...
    // public void addCompoundDocumentFileDefinitions(DocumentModel compoundDoc, String uid, String version, String
